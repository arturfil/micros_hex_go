...
func (h *PaymentHTTPHandler) registerRoutes(router *chi.Mux) {
    router.Get("/webhook", h.handleCheckoutWebhook)
}
...
func (h *PaymentHTTPHandler) handleCheckoutWebhook(w http.ResponseWriter, r *http.Request) {
    ...
    if event.Type == "checkout.session.completed" {
        ...
        if session.PaymentStatus == "paid" {
            log.Printf("Payment for Checkout Session %v succeded!", session.ID)
        }
    }
